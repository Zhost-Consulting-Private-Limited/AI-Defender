...
from .models import Tenant, Agent, RiskScore, Incident, Policy, Command, AuditLog, HourlyReport, SIEMWebhook
from .schemas import AgentEnrollRequest, EventBatch, PolicyInput, CommandInput, SIEMWebhookInput
...
@app.post("/api/v1/integrations/siem/webhooks", dependencies=[Depends(require_role("admin"))])
def add_siem_webhook(payload: SIEMWebhookInput, session: Session = Depends(get_session)):
    w = SIEMWebhook(**payload.model_dump())
    session.add(w)
    add_audit(session, payload.tenant_id, "admin", "create_siem_webhook", "integration", payload.name)
    session.commit()
    session.refresh(w)
    return w


@app.get("/api/v1/integrations/siem/webhooks", dependencies=[Depends(require_role("admin", "analyst"))])
def list_siem_webhooks(tenant_id: int, session: Session = Depends(get_session)):
    return session.exec(select(SIEMWebhook).where(SIEMWebhook.tenant_id == tenant_id)).all()


@app.delete("/api/v1/integrations/siem/webhooks/{webhook_id}", dependencies=[Depends(require_role("admin"))])
def delete_siem_webhook(webhook_id: int, session: Session = Depends(get_session)):
    hook = session.get(SIEMWebhook, webhook_id)
    if not hook:
        raise HTTPException(status_code=404, detail="webhook not found")
    tenant_id = hook.tenant_id
    session.delete(hook)
    add_audit(session, tenant_id, "admin", "delete_siem_webhook", "integration", str(webhook_id))
    session.commit()
    return {"ok": True}
